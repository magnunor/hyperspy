...
LAZY_RESULT_ARG = \
    """lazy_result : None or bool
           If ``True``, the result will be returned as a lazy signal. This means
           the calculation itself will be delayed until either compute() is used,
           or the signal is stored as a file.
           If ``False``, the result will be returned as a non-lazy signal, this
           means the results will be calculated directly, and loaded into memory.
           If ``None`` the result will be lazy if the input signal is lazy, and
